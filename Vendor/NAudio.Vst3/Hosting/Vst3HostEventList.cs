...
{
    private const int ExpectedEventSize = 48;
    private readonly List<Event> _events = new();

    public Vst3HostEventList()
    {
        var actualSize = Marshal.SizeOf<Event>();
        if (actualSize != ExpectedEventSize)
        {
            throw new TypeLoadException(
                $"La estructura VST3 Event mide {actualSize} bytes; debe medir {ExpectedEventSize} en Windows x64.");
        }
    }
